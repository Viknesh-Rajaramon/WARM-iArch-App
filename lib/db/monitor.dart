...
class Monitor {
  ...
  const Monitor(this.serialNumber, this.locationId, this.name, this.projectId);
...
Future<List<int>> getLocationIdsByProjectId(String projectId) async {
  if (projectId.isEmpty) {
    return [];
  }
    
  final (monitors, status) = await getMonitorsByProjectId(projectId);
  return status == HttpStatus.found ? [for (var monitor in monitors) monitor.locationId] : [];
}
